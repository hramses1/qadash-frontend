        ...
        <div v-else class="table-scroll">
          <table class="data-table">
            <thead>
              <tr>
                <th class="idx-col">#</th>
                <th v-for="c in columns" :key="c">{{ prettyCol(c) }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(row, i) in visibleRows" :key="i">
                <td class="idx-col">{{ i + 1 }}</td>
                <td v-for="c in columns" :key="c" :title="row[c]">{{ row[c] }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.idx-col {
  width: 1%; text-align: right; opacity: .5;
  font-variant-numeric: tabular-nums; padding-right: .9rem;
}
.data-table tbody tr:nth-child(even) { background: rgba(100,116,139,.04); }
